...
- **REQ-024**: ユーザーがローカルモード設定を有効にした時、システムは外部API（クラウドDB等）への接続を一切行ってはならない
- **REQ-025**: システムはローカル埋め込みモデル（ONNX Runtime経由）を使用しなければならない
...
- **REQ-030**: ユーザーが設定ファイルを作成していない時、システムはデフォルト設定（ローカルモード、Milvus standalone、ローカルONNXモデル）で起動しなければならない
- **REQ-031**: ユーザーがClaude CodeのMCP設定で環境変数を指定した時、システムはその環境変数を優先して使用しなければならない
- **REQ-032**: システムは以下の環境変数をサポートしなければならない：LSP_MCP_MODE、LSP_MCP_VECTOR_BACKEND、LSP_MCP_VECTOR_ADDRESS、LSP_MCP_VECTOR_TOKEN
- **REQ-033**: もし設定ファイルと環境変数の両方が存在する場合、システムは「環境変数 > 設定ファイル > デフォルト設定」の優先順位で設定を適用しなければならない
- **REQ-034**: システムは起動時に適用された設定内容（モード、ベクターDB）をログに出力しなければならない
...
- **NFR-008**: システムはRust 1.75以降（MSRV）でコンパイル可能でなければならない（推奨: Rust 1.80以降）
- **NFR-039**: システムは単一バイナリとして配布可能でなければならない（外部ランタイムへの依存なし）
- **NFR-040**: リリースビルドの実行ファイルサイズは100MB以下でなければならない（埋め込みモデルファイルを除く）
...
- **NFR-017**: システムはローカル埋め込みモデル（ONNX Runtime経由）をサポートしなければならない
...
- **NFR-037**: システムは分散トレーシングのコンテキスト伝播をサポートし、外部サービス（ベクターDB）へのリクエストにトレースコンテキストを含めなければならない
...
- **クラウドモード**: クラウドベースのベクターDB（Zilliz Cloud）を利用するモード
- **ローカル埋め込みモデル**: デバイス上で実行される機械学習モデル（ONNXフォーマット、tract/ort crateで実行）
