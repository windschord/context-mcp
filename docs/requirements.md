# 要件定義：LSP-MCP（Language Server Protocol based MCP）

## 概要

LSP-MCPは、ソースコードとドキュメントを統合的に解析し、セマンティック検索を提供するModel Context Protocol (MCP)プラグインです。Tree-sitterによるAST解析とベクターデータベースを組み合わせることで、Claude Codeに最適化されたコンテキスト理解と効率的なコード検索を実現します。
...
LSP-MCPは、Claude Contextのコンセプトを基盤としながら、以下の点でClaude Code環境に最適化した独自の発展を目指します：
...
- **REQ-025**: ローカルモード時、システムはローカル埋め込みモデル（Transformers.js、ONNX等）を使用しなければならない
- **REQ-026**: ローカルモード時、システムはローカルベクターDB（Chroma、SQLite+拡張等）にデータを保存しなければならない
- **REQ-027**: システムは初回セットアップ時に、ローカルモードまたはクラウドモードの選択肢を提供しなければならない
- **REQ-028**: ローカルモード選択時、システムは必要なローカルモデルとDBを自動的にダウンロード・セットアップしなければならない
...
- **NFR-010**: システムは埋め込みモデル（OpenAI、VoyageAI、ローカルモデル等）を設定で切り替え可能でなければならない
- **NFR-011**: システムはベクターDBバックエンド（Milvus、Qdrant、Chroma等）をプラグイン方式で切り替え可能でなければならない
...
- **NFR-017**: システムはローカル埋め込みモデル（Transformers.js、ONNX Runtime等）をサポートしなければならない
- **NFR-018**: システムはローカルベクターDB（Chroma、DuckDB、SQLite+ベクトル拡張等）をサポートしなければならない
- **NFR-019**: システムはローカルモード時の性能が、クラウドモードと比較して実用的な範囲（2倍以内の処理時間）でなければならない
- **NFR-020**: システムはインターネット接続なしでも基本機能（インデックス化、検索）が動作しなければならない
- **NFR-021**: システムはローカルモデルのダウンロード・更新を、ユーザーの明示的な許可を得た上で実行しなければならない

### ユーザビリティ要件

- **NFR-022**: システムは設定ファイル（.lsp-mcp.json）により、除外パターン、言語設定、モデル選択を簡単にカスタマイズできなければならない
...
- ローカル埋め込みモデル（Transformers.js、ONNX Runtime等）

### オプション依存関係（代替ベクターDB）
- Chroma（軽量ローカルDB、Docker不要）
- DuckDB（SQLベース、ローカルDB）
