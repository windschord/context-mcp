...
> **注記**: このドキュメントはNode.js/TypeScript実装のプラグインシステムを説明しています。現在のRust実装（v0.x）では、プラグインシステムは簡素化されており、Milvusベクターストアのみをサポートしています。新しいバックエンドの追加には、Rustコードの直接編集が必要です。

---

## 概要

Context-MCPは、ベクターストアと埋め込みエンジンのプラグインアーキテクチャを採用しており、新しいバックエンドを簡単に追加できます。このガイドでは、プラグインの開発方法を説明します（Node.js/TypeScript実装向け）。
...
`.context-mcp.json` でプラグインを指定できるようにします。
