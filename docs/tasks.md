...
### フェーズ8: ゼロコンフィグ対応とドキュメント整備
*推定期間: 2日*

#### タスク8.1: docs/requirements.mdへのゼロコンフィグ要件追加
**説明**: ストーリー7（ゼロコンフィグセットアップ）と関連する非機能要件を追加
**受入基準**:
- [x] ユーザーストーリー7「ゼロコンフィグセットアップ」が追加されている
- [x] 受入基準REQ-030〜REQ-034が記載されている
- [x] 非機能要件NFR-029〜NFR-031が追加されている
- [x] EARS記法に準拠している
...
#### タスク8.2: docs/design.mdへの環境変数ベース設定設計追加
**説明**: コンポーネント9（環境変数ベース設定システム）と技術的決定事項6を追加
**受入基準**:
- [x] コンポーネント9「Environment-based Configuration System」が追加されている
- [x] サポート環境変数一覧が記載されている
- [x] 設定の優先順位が明確に記載されている
- [x] 技術的決定事項6「ゼロコンフィグ設計」が追加されている
- [x] ユーザー体験の具体例が含まれている
...
#### タスク8.3: docs/tasks.mdへのフェーズ8追加
**説明**: 本フェーズのタスク定義を追加
**受入基準**:
- [x] フェーズ8が追加されている
- [x] タスク8.1〜8.6が定義されている
- [x] 各タスクに受入基準と推定工数が記載されている

**依存関係**: タスク8.2
**推定工数**: 0.5時間
**ステータス**: `DONE`

#### タスク8.4: README.mdのクイックスタート書き換え
**説明**: ゼロコンフィグ手順を最上位に配置し、設定ファイル作成を任意化
**受入基準**:
- [ ] クイックスタートセクションが書き換えられている
- [ ] Docker Compose起動 + MCP設定のみの手順が最初に記載されている
- [ ] 設定ファイル作成が「オプション」として記載されている
- [ ] 環境変数を使ったMCP設定例が充実している

**依存関係**: タスク8.3
**推定工数**: 2時間
**ステータス**: `TODO`

#### タスク8.5: docs/ENVIRONMENT_VARIABLES.md作成
**説明**: 環境変数の完全なリファレンスドキュメント作成
**受入基準**:
- [ ] すべてのサポート環境変数が記載されている
- [ ] 各環境変数の説明、型、デフォルト値、使用例が含まれている
- [ ] 設定の優先順位が説明されている
- [ ] ユースケース別の設定例が含まれている
- [ ] トラブルシューティングセクションがある
...
#### タスク8.6: docs/SETUP.mdへの環境変数セクション追加
**説明**: セットアップガイドにゼロコンフィグ手順を追加
**受入基準**:
- [ ] 「ゼロコンフィグモード」セクションが追加されている
- [ ] 環境変数ベースのセットアップ手順が記載されている
- [ ] Claude Code設定例（環境変数のみ）が含まれている
- [ ] 従来の設定ファイル方式との比較表がある
